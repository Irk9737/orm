...
func (orm *OrmValue[T]) GetDB() error {
	sqlStr := "root:2583585355@tcp(127.0.0.1:3306)/xorm?charset=utf8mb4&parseTime=true&loc=Local" // xorm: 数据库名称
	var err error
	// 1、创建数据库引擎
...
func (orm *OrmValue[T]) QueryData(c *gin.Context) ([]T, error) {
	stuNum := c.Query(orm.Cfg.Business.Key)
	var types []T
	var sb strings.Builder
	sb.WriteString(orm.Cfg.Business.Key + "=?")
	//err := x.Where(sb.String(), stuNum).Find(&types)
